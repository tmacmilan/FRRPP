function [ cOptimal,gammaOptimal,alphaOptimal,beta ] = learnFRRPPForEach2( TAll,Feature,indicatorT,e0 )
u = 8000;
iterMax=500;
stepC=0.000001;
stepGamma=0.00001;
...
for i=1:iterMax
    ...
    %cList=exp(Feature*beta)'*0.2+C(i+1,:)*0.8;
    cList =C(i+1,:) + (exp(Feature*beta)'-C(i+1,:))*0.1;
    cList(cList<0,:)=0.001;
    C(i+1,:)= cList;
    for p=1:nPost
        T = indicatorTimeList{p};
